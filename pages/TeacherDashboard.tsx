...
import { BattleService, type BattleRoom } from '../services/battleService';
...
const TeacherDashboard: React.FC = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [quizzesData, battleRoomsData] = await Promise.all([
        QuizService.getAllQuizzes(),
        BattleService.getTeacherBattleRooms()
      ]);
      setQuizzes(quizzesData);
      setBattleRooms(battleRoomsData);
    } catch (error) {
  ...
  const copyQuizLink = async (slug: string) => {
    ...
    try {
      ...
      showToast.info(`Link đề thi: ${url}`, 8000);
    }
  };

  const handleTakeQuiz = (quiz: SavedQuiz) => {
    // Chuyển đến trang làm bài với dữ liệu quiz
    navigate('/exam', {
      state: {
        questions: quiz.questions,
        title: `${quiz.title} (Giáo viên xem trước)`,
        quizId: quiz.id,
  ...
  const features = [
    ...
    },
    {
      ...
      description: "Xem thống kê chi tiết về đề thi và hoạt động giảng dạy",
      icon: <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="mt-16">
                ...
                >
                  ...
                  <div className="space-y-2">
                    <button
                      ...
                      className="w-full flex items-center justify-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-semibold"
                    >
          ...
          <div className="bg-white rounded-xl shadow-lg p-8 mb-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-gray-900">⚔️ Phòng thi đấu của bạn</h2>
              <Link
                to="/teacher/battle/create"
            ...
            {battleRooms.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 mb-4">Chưa có phòng thi đấu nào</p>
                <Link
                  to="/teacher/battle/create"
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600 mb-3">
                          Mã phòng: <span className="font-mono font-bold text-red-600">{room.room_code}</span>
                        </p>
                        <div className="flex items-center text-xs text-gray-500 space-x-4">
                          <span>📊 {room.max_participants} người</span>
                          <span>⏱️ {room.question_time_limit}s/câu</span>
                        </div>
                      </div>
                      <span className={`px-2 py-1 text-xs rounded-full ${
                        ...
                        {room.status === 'waiting' ? 'Chờ' :
                         room.status === 'active' ? 'Đang thi' : 'Kết thúc'}
                      </span>
                    </div>

                    <div className="space-y-2">
                      <Link
                        to={`/battle/room/${room.room_code}`}
